...
//var pieColors = ["#11d5db","#4393c3", "#fddbc7"];
//var pieColors = ["#898989", "#f26763", "#be140e"];
//var pieColors = ["#11d5db","#4393c3", "#fddbc7"];
...
function buildProvinceDropdown() {
    ...
    for (var i = 0; i < provinceList.length; i++) {
        ...
        var listItemHtml = '<li><a href="#" onClick="provinceSelect(' + "'" +
            item + "'" + '); return false;">' + item + "</li>"
...
function municipalitySelect(municipality) {
    ...
    activeBarangay = "ALL";
    $("#selected-admin-label").html(activeProvince + ", " +
        activeMunicipality);
...
function barangaySelect(barangay) {
    ...
    $("#selected-admin-label").html(activeProvince + ", " +
        activeMunicipality + ", " + activeBarangay);
    analyzeData();
}
...
function buildMunicipalityDropdown() {
    ...
    $.each(surveyData, function(index, survey) {
        ...
        if ($.inArray(thisMunicipality, municipalityList) === -1 &&
            survey["province"] === activeProvince) {
    ...
    for (var i = 0; i < municipalityList.length; i++) {
        ...
        var listItemHtml = '<li><a href="#" onClick="municipalitySelect(' +
            "'" + item + "'" + '); return false;">' + item + "</li>"
...
function buildBarangayDropdown() {
    ...
    $.each(surveyData, function(index, survey) {
        ...
        if ($.inArray(thisBarangay, barangayList) === -1 && survey[
            "province"] === activeProvince && survey[
    ...
    for (var i = 0; i < barangayList.length; i++) {
        ...
        var listItemHtml = '<li><a href="#" onClick="barangaySelect(' + "'" +
            item + "'" + '); return false;">' + item + "</li>"
...
function analyzeData() {
    ...
    filteredData = [];
    $.each(surveyData, function(index, survey) {
        if (survey["province"] === activeProvince || activeProvince ===
            "ALL") {
            if (survey["municipality"] === activeMunicipality ||
                activeMunicipality === "ALL") {
                if (survey["barangay"] === activeBarangay ||
                    activeBarangay === "ALL") {
...
function BC1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_profile'></span>Respondents' Profile</h3><hr>"
    );
    var questionID = "BC1";
    var questionEnglish = "Number of people in household";
    var questionTagalog = "Bilang ng tao sa isang sambahayan";
    $("#infoWrapper").append('<div class="row"><div id="' + questionID +
        '_info" class="box-info"></div></div><hr>');
    var infoSelector = "#" + questionID + "_info";
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
            false) ? "<br><small>" + questionTagalog + "</small>" : "") +
        "</h4>" + '<table class="table table-custom"><thead>' + '<tr>' +
        '<th></th>' +
        '<th>Female <span class="text-tagalog">[Babae]</span></th>' +
        '<th>Male <span class="text-tagalog">[Lalaki]</span></th>' +
        '</tr>' + '</thead>' + '<tbody>' + '<tr class="active">' +
        '<td style="font-weight:bold;"><span id="grandTotal"></span> total</td>' +
        '<td><span id="femaleTotal"></span></td>' +
        '<td><span id="maleTotal"></span></td>' + '</tr>' + '<tr>' +
        ...
        '<td><span id="maleInfant"></span></td>' + '</tr>' + '<tr>' +
        ...
        '<td><span id="maleYoung"></span></td>' + '</tr>' + '<tr>' +
        ...
        '<td><span id="maleChild"></span></td>' + '</tr>' + '<tr>' +
        ...
        '<td><span id="maleAdult"></span></td>' + '</tr>' + '<tr>' +
        ...
        '<td><span id="maleSenior"></span></td>' + '</tr>' +
        '</tbody></table>';
    ...
    var femaleTotal = femaleInfant + femaleYoung + femaleChild +
        femaleAdult + femaleSenior;
    var maleTotal = maleInfant + maleYoung + maleChild + maleAdult +
        maleSenior;
...
function BC2() {
    ...
    $("#infoWrapper").append('<div class="row"><div id="' + questionID +
        '" class="box-chart"><svg id="' + questionID +
        '_chart"></svg></div><div id="' + questionID +
        '_info" class="box-info"></div></div><hr>');
    var width = 180;
    var chart = nv.models.pie().width(width - 60).height(width - 60).x(
        function(d) {
            return d.key
        }).y(function(d) {
        return d.y
    }).color(pieColors).showLabels(true);
    var chartSelector = "#" + questionID + "_chart";
    d3.select(chartSelector).datum(thisPieData).transition().duration(1200)
        .attr('width', width).attr('height', width).call(chart);
    var el = $(".nv-pieLabels");
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
            false) ? "<br><small>" + questionTagalog + "</small>" : "") +
        "</h4>" + "<p><strong>" + totalCount + " respondents</strong><br>" +
        "<span class='percText-1'>" + malePerc +
        "</span> male <span class='text-tagalog'>[lalaki]</span> (" +
        maleCount.toString() + ")<br>" + "<span class='percText-2'>" +
        femalePerc +
        "</span> female <span class='text-tagalog'>[babae]</span> (" +
        femaleCount.toString() + ")<br>" + "<span class='percText-3'>" +
        noResponsePerc +
        "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
        skipped.toString() + ")<br>";
    thisInfoHtml += "</p>";
    $(infoSelector).append(thisInfoHtml);
    BC3();
...
function BC3() {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisInfoHtml = "";
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<strong>" + yearResponses.length
        .toString() + " respondents providing # of years</strong><br>" +
        "Average years: " + avgYears.toString() + "<br>" + "Min: " +
        minYears.toString() + " / Max: " + maxYears.toString() + "<br>";
    $(infoSelector).append(thisInfoHtml);
    FA1();
}
...
function FA1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_firstaid'></span>Topic: First Aid</h3><hr>"
    );
    var questionID = "cbhfa-FA1";
    var questionEnglish =
        "Have you ever attended any training program to learn basic first aid?";
    var questionTagalog =
        "Nakadalo ka na ba ng pagsasanay patungkol sa paunang lunas?";
...
function FA2() {
    ...
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>Of the " +
        totalAttended.toString() +
        " respondents attending a training program to learn basic first aid:</strong><br>" +
        "<span class='percText-1'>" + less2Perc +
        "</span> did so in the last 2 years (" + less2years.toString() +
        ")<br>" + "<span class='percText-2'>" + more2Perc +
        "</span> did so more than 2 years ago (" + more2years.toString() +
        ")<br>" + "<span class='percText-3'>" + dkskipPerc +
        "</span> don't know <span class='text-tagalog'>[hindi alam]</span> (" +
        dkPerc + ", " + dontKnow.toString() +
        ") or no response <span class='text-tagalog'>[walang sagot]</span> (" +
        noResponsePerc + ", " + noResponse.toString() + ")<br>" + "(" +
        noTraining.toString() + ((noTraining == 1) ? " respondent has" :
            " respondents have") + " not attended a training" + ")</p><br>";
    $(infoSelector).append(thisInfoHtml)
    FA3();
}
...
function FA3() {
    ...
    analysisSelectOneWhatAnswer(questionID, questionEnglish,
        questionTagalog, answersEnglish, answersTagalog);
    FA5();
}
...
function FA5() {
    ...
    var questionEnglish =
        "What will be your first action if you see someone is bleeding?";
    var questionTagalog =
        "Ano ang iyong magiging unang pagkilos o gagawing pangunang lunas kapag ikaw ay makakakita ng taong nagdurugo?";
    var keyInterventionEnglish = "put pressure to stop bleeding";
    var keyInterventionTagalog =
        "lagyan ng pressure o diin ang apektadong bahagi para huminto ang pagdurugo";
    var know = 0;
    var other = 0;
    var dk = 0;
    var skip = 0;
    ...
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        ...
        totalAttended.toString() +
        " respondents attending a training program to learn basic first aid:</strong><br>" +
        "<span class='percText-1'>" + knowPerc + "</span> " +
        keyInterventionEnglish + " <span class='text-tagalog'>[" +
        keyInterventionTagalog + "]</span> (" + know.toString() + ")<br>" +
        "<span class='percText-2'>" + otherPerc +
        "</span> gave some other answer (" + other.toString() + ")<br>" +
        "<span class='percText-3'>" + dkskipPerc +
        "</span> don't know <span class='text-tagalog'>[hindi alam]</span> (" +
        dkPerc + ", " + dk.toString() +
        ") or no response <span class='text-tagalog'>[walang sagot]</span> (" +
        skipPerc + ", " + skip.toString() + ")<br>" + "(" + notAsked.toString() +
        ((notAsked == 1) ? " respondent" : " respondents") +
        " have not attended a training";
    thisInfoHtml += ")</p><br>";
    $(infoSelector).append(thisInfoHtml);
    FA6();
}
...
function FA6() {
    ...
    var questionEnglish =
        "What will be your first action if you see someone has been burnt?";
    var questionTagalog =
        "Ano ang iyong magiging unang tugon/aksyon kapag nakakita ka ng taong napaso?";
    var keyInterventionEnglish = "put cold clean water on the burned area";
    var keyInterventionTagalog =
        "lagyan ng malamig at malinis na tubig sa napasong bahagi";
    var know = 0;
    var other = 0;
    var dk = 0;
    var skip = 0;
...
function FA7() {
    ...
    var questionEnglish =
        "Did you at any occasion in the last year injure yourself and was given first aid by a volunteer?";
    var questionTagalog =
        "Mayroon bang naging pagkakataon kung saan nasaktan mo ang iyong sarili at nilapatan ka ng pangunang lunas ng isang volunteer?";
...
function CM1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_majoremergencies'></span>Topic: Community Mobilization in Major Emergencies</h3></div><hr>"
    );
    var questionID = "CM1";
    var questionEnglish =
        "What would you do to respond safely to a disaster?";
    var questionTagalog =
        "Ano ang iyong gagawin upang makatugon ng ligtas sa isang kalamidad?";
    var answersEnglish = {
    ...
    var optionCount = 9;
    analysisMoreThreeLessThree(questionID, questionEnglish, questionTagalog,
        answersEnglish, answersTagalog, optionCount);
    CM2();
}
...
function CM2() {
    ...
    var questionEnglish =
        "Did you receive psychosocial support from a volunteer following the disaster?";
    var questionTagalog =
        "Nakatanggap ka ba ng psychosocial support mula sa isang volunteer matapos ang isang kalamidad?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    safemotherhood();
}
...
function SM1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_safemotherhood'></span>Topic: Safe Motherhood</h3><hr>"
    );
    var questionID = "SM1";
    var questionEnglish =
        "During your last pregnancy, did you see anyone for antenatal care?";
    var questionTagalog =
        "Sa panahon ng iyong huling pagbubuntis, kumunsulta ka ba ng isang healthcare worker para mapangalagaan ang iyong pagbubuntis?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    SM2();
}
...
function SM2() {
    ...
    var thisInfoHtml = "";
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>";
    $(infoSelector).append(thisInfoHtml);
    $(infoSelector).append("<strong>" + askedCount.toString() +
        " respondents (multiple responses possible)</strong><br>");
    for (response in allResponses) {
        var thisResponseCount = allResponses[response];
        var thisResponsePerc = formatPerc(allResponses[response] /
            askedCount);
        ...
        if (thisResponseEng !== thisResponseTag) {
            thisHtml += " <span class='text-tagalog'>[" + thisResponseTag +
                "]</span>";
        }
        thisHtml += " (" + thisResponseCount + ")<br>";
        $(infoSelector).append(thisHtml);
    }
    $(infoSelector).append("(" + notAskedCount +
...
function SM3() {
    ...
    var optionCount = 9;
    var questionEnglish =
        "During your last pregnancy, where did you receive antenatal care?";
    var questionTagalog =
        "Sa panahon ng iyong pagbubuntis , saan ka nagpakonsulta para mapangalagaan ang iyong pagbubuntis?";
    ...
    $.each(filteredData, function(surveyIndex, survey) {
        if (survey[dk] === "n/a") {
            ...
            if (survey["SM3-A"] === "TRUE" || survey["SM3-B"] ===
                "TRUE" || survey["SM3-C"] === "TRUE") {
                homeCare++;
            }
            if (survey["SM3-D"] === "TRUE" || survey["SM3-E"] ===
                "TRUE" || survey["SM3-F"] === "TRUE" || survey[
                    "SM3-G"] === "TRUE") {
                publicCare++;
            }
            if (survey["SM3-H"] === "TRUE" || survey["SM3-I"] ===
                "TRUE") {
                privateCare++;
            }
        }
    ...
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>" + totalCount +
        " respondents (may have received care in more than one sector)</strong><br>" +
        "<span class='percText-other1'>" + homePerc +
        "</span> received care in home" + " (" + homeCare.toString() +
        ")<br>" + "<span class='percText-other1'>" + publicPerc +
        "</span> received care at a public facility" + " (" + publicCare.toString() +
        ")<br>" + "<span class='percText-other1'>" + privatePerc +
        "</span> received care at a private facility" + " (" + privateCare.toString() +
        ")</p>";
    $(infoSelector).append(thisInfoHtml);
    $(infoSelector).append(
        "<strong>Raw counts of responses (multiple responses possible)</strong><br>"
    );
    for (response in allResponses) {
        var thisResponseCount = allResponses[response];
        var thisResponsePerc = formatPerc(allResponses[response] /
    ...
    $(infoSelector).append("(" + topicSkipped.toString() +
        " not asked this question)");
    SM4();
}
...
function SM4() {
    ...
    var questionEnglish =
        "During your last pregnancy, how many months pregnant were you when you first received antenatal care?";
    var questionTagalog =
        "Sa panahon ng iyong pagbubuntis, ilang buwan kang buntis noong una kang magpakonsulta sa isang healthcare worker?";
    var monthResponses = [];
    var dkCount = 0;
    ...
    var thisInfoHtml = "";
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<strong>" + monthResponses.length
        .toString() + " respondents providing # of months</strong><br>" +
        "Average months: " + avgMonths.toString() + "<br>" + "Min: " +
        minMonths.toString() + " / Max: " + maxMonths.toString() + "<br>" +
        "(" + dkCount.toString() +
        " don't know <span class='text-tagalog'>[hindi alam]</span>, " +
        noResponseCount.toString() +
        " no response <span class='text-tagalog'>[walang sagot]</span>, " +
...
function SM5() {
    ...
    var questionEnglish =
        "During your last pregnancy, how many times did you receive antenatal care?";
    var questionTagalog =
        "Sa panahon ng iyong huling pagbubuntis, ilang beses ka kumonsulta sa isang healthcare worker?";
    var numberResponses = [];
    var dkCount = 0;
    var noResponseCount = 0;
    var notAskedCount = 0;
    ...
    var thisInfoHtml = "";
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<strong>" + numberResponses.length
        .toString() + " respondents providing # of times</strong><br>" +
        "Average Times: " + avgTimes.toString() + "<br>" + "Min: " +
        minTimes.toString() + " / Max: " + maxTimes.toString() + "<br>" +
        "(" + dkCount.toString() +
        " don't know <span class='text-tagalog'>[hindi alam]</span>, " +
        noResponseCount.toString() +
        " no response <span class='text-tagalog'>[walang sagot]</span>, " +
...
function SM6() {
    ...
    var questionTagalog =
        "Bilang bahagi ng pangangalaga sa iyong pagbubuntis, alin man sa mga sumusunod ay nagawa isa o higit pa?";
    var heightYes = 0;
    var heightNo = 0;
    var heightSkip = 0;
    var bpYes = 0;
    ...
    $("#infoWrapper").append('<div class="row"><h4>' + questionEnglish +
        '<br><small>' + questionTagalog + '</small></h4><p><strong>' +
        totalCount.toString() +
        ' respondents reporting having received antenatal care</strong></p>' +
        '</div><div class="row"><div class="col-sm-6">' +
        '<h4>Was your height taken? <br><small>Sinukat ba ang iyong laki/tangkad?</small></h4>' +
        '<div id="' + heightID +
        '" class="box-chart-no-float"><svg id="' + heightID +
        '_chart"></svg></div><div id="' + heightID +
        '_info" class="box-info"></div></div>' +
        '<div class="col-sm-6">' +
        '<h4>Was your blood pressure measured? <br><small>Sinukat ba ang iyong blood pressure?</small></h4>' +
        '<div id="' + bpID + '" class="box-chart-no-float"><svg id="' +
        bpID + '_chart"></svg></div><div id="' + bpID +
        '_info" class="box-info"></div></div></div>' +
        '<div class="row"><div class="col-sm-6">' +
        '<h4>Did you give a urine sample? <br><small>Nagbigay kaba ng sample ng iyong ihi?</small></h4>' +
        '<div id="' + urineID +
        '" class="box-chart-no-float"><svg id="' + urineID +
        '_chart"></svg></div><div id="' + urineID +
        '_info" class="box-info"></div></div>' +
        '<div class="col-sm-6">' +
        '<h4>Did you give a blood sample? <br><small>Nagbigay k aba ng sample ng iyong dugo?</small></h4>' +
        '<div id="' + bloodID +
        '" class="box-chart-no-float"><svg id="' + bloodID +
        '_chart"></svg></div><div id="' + bloodID +
        '_info" class="box-info"></div></div></div>' + '<hr>');
    $.each(["SM6_A", "SM6_B", "SM6_C", "SM6_D"], function(index, questionID) {
        var width = 180;
        var chart = nv.models.pie().width(width - 60).height(width -
            60).x(function(d) {
            return d.key
        }).y(function(d) {
            return d.y
        ...
        switch (questionID) {
            case "SM6_A":
                thisPieData = pieData(heightYes, heightNo,
                    heightSkip);
                yesCount = heightYes;
                noCount = heightNo;
        ...
        d3.select(chartSelector).datum(thisPieData).transition().duration(
            1200).attr('width', width).attr('height', width).call(
            chart);
        var el = $(".nv-pieLabels");
        $.each(el, function(aIndex, a) {
            a.parentNode.appendChild(a);
        ...
        var noPerc = formatPerc(noCount / (yesCount + noCount +
            skipCount));
        var skipPerc = formatPerc(skipCount / (yesCount + noCount +
            skipCount));
        thisInfoHtml = "<p><span class='percText-1'>" + yesPerc +
            "</span> yes <span class='text-tagalog'>[oo]</span> (" +
            yesCount.toString() + ")<br>" +
            "<span class='percText-2'>" + noPerc +
            "</span> no <span class='text-tagalog'>[hindi]</span> (" +
            noCount.toString() + ")<br>" +
            "<span class='percText-3'>" + skipPerc +
            "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
...
function SM7() {
    var questionID = "SM7";
    var questionEnglish =
        "During your Last pregnancy did you receive an injection in the arm to prevent the baby from getting tetanus that is convulsions after birth?";
    var questionTagalog =
        "Sa panahon ng iyong pagbubuntis, nabigyan ka ba ng bakuna laban sa tetanus upang maiwasan ng iyong sanggol ang pagkakaroon nito?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    SM8();
}
...
function SM8() {
    var questionID = "SM8";
    var questionEnglish =
        "While pregnant, how many times did you receive such an injection?";
    var questionTagalog =
        "Habang ikaw ay buntis, ilang beses ka nabigyan ng nasabing bakuna?";
    var answersEnglish = {
        "A": "one",
        "B": "two",
        "C": "three or more",
...
function SM9() {
    ...
    analysisSelectMultipleWhatAnswers(questionID, questionEnglish,
        questionTagalog, answersEnglish, answersTagalog);
    SM10();
}
...
function SM10() {
    var questionID = "SM10";
    var questionEnglish =
        "During pregnancy, women may encounter severe problems or illnesses and should go or be taken immediately to a health facility. What types of symptoms would cause you to seek immediate care at a health facility (right away)?";
    var questionTagalog =
        "Sa panahon ng pagbubuntis , ang mga babae ay maaaring magkaroon ng problema o matinding karamdaman at kinakailangang pumunta o agarang dalhin sa sa isang pagamutan. Anong uri ng mga sintomas ang dahilan ng agarang pagkonsulta sa isang pagamutan?";
    var answersEnglish = {
...
function SM11() {
    var questionID = "SM11";
    var questionEnglish =
        "After your last child was born were you and your baby seen by anyone for postnatal care within the next two days?";
    var questionTagalog =
        "Matapos ipanganak ang inyong bunsong anak, ikaw ba at ang iyong sanggol ay nakita, natignan ng isang health worker sa loob ng 2 araw?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    SM12();
}
...
function NB1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_newborn'></span>Topic: Care of a Newborn</h3><hr>"
    );
    var questionID = "NB1";
    var questionEnglish =
        "What are the important things for home based care of a newborn baby (immediately when born)?";
    var questionTagalog =
        "Ano ang mga mahahalagang bagay na dapat isaalang-alang sa pangangalaga ng bagong silang na sanggol sa bahay?";
    var answersEnglish = {
...
function NB6() {
    var questionID = "NB6";
    var questionEnglish =
        "Sometimes newborns have severe illnesses within the first month of life and should be taken immediately to a health facility. What types of symptoms would cause you to take your newborn to a health facility right away?";
    var questionTagalog =
        "Minsan ang mga bagong panganak na saggol ay maaaring magkaroon ng matinding karamdaman sa loob lamang ng 1 buwan pagkapanganak kailangang madala agad sa isang pagamutan upang masuri. Anong uri ng mga sintomas ang dahilan ng agarang pagkonsulta sa isang pagamutan?";
    var answersEnglish = {
        "NB6-A": "convulsions",
        "NB6-B": "high fever",
...
function NB3() {
    var questionID = "NB3";
    var questionEnglish =
        "How long after birth did you first put baby to the breast?";
    var questionTagalog =
        "Pagkapanganak, gaano katagalbago mo pinasuso ng iyong gatas ang iyong sanggol?";
    var answersEnglish = {
        "immediate": "immediate",
        "hours": "hours",
        "days": "days",
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> If less than 24 hours record 'hours', otherwise record 'days'</small><br>";
...
function NB4() {
    var questionID = "NB4";
    var questionEnglish =
        "Did you give the baby the first liquid (Colostrum) that came from your breasts?";
    var questionTagalog =
        "Binigyan mo ba ang sanggol ng unang patak ng iyong gatas (Colostrum)?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NB5();
}
...
function NB5() {
    var questionID = "NB5";
    var questionEnglish =
        "In the first three days after delivery, was the baby given anything to drink other than breast milk?";
    var questionTagalog =
        "Sa unang tatlong araw matapos mong manganak, binigyan mo ba ng anumang/ibang inumin ang iyong sanggol maliban sa iyong gatas?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
...
function NU2() {
    var questionID = "NU2";
    var questionEnglish =
        "For how many months did you breastfeed your last baby?";
    var questionTagalog =
        "Ilang buwan mo pinasuso ng iyong gatas ang iyong bunsong anak?";
    var monthResponses = [];
    var dkCount = 0;
    var noResponseCount = 0;
    var notAskedCount = 0;
    ...
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        ...
        "Average months: " + avgMonths.toString() + "<br>" + "Min: " +
        minMonths.toString() + " / Max: " + maxMonths.toString() + "<br>" +
        "(" + dkCount.toString() + " don't know, " + noResponseCount.toString() +
        " no response, " + notAskedCount.toString() +
        " not asked this question)";
    $(infoSelector).append(thisInfoHtml);
    babynutrition();
}
...
function NU3breastmilk() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_babynutrition'></span>Topic: Baby Nutrition</h3><hr>"
    );
    var questionID = "NU3_breastmilk";
    var questionEnglish =
        "Now I would like to ask you about liquids or foods children had yesterday during the day or at night. Did your child/children drink/eat: <u>Breast milk</u>?";
    var questionTagalog =
        "Ano-anong uri ng pagkain at inumin ang binigay mo sa iyong mga anak kahapon mula umaga hanggang gabi? Kumain o uminom ang iyong mga anak ng: <u>Gatas ng ina</u>?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NU3plainwater();
}
...
function NU3plainwater() {
    ...
    var questionEnglish =
        "Now I would like to ask you about liquids or foods children had yesterday during the day or at night. Did your child/children drink/eat: <u>Plain water</u>?";
    var questionTagalog =
        "Ano-anong uri ng pagkain at inumin ang binigay mo sa iyong mga anak kahapon mula umaga hanggang gabi? Kumain o uminom ang iyong mga anak ng: <u>Tubig</u>?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NU3formula();
}
...
function NU3formula() {
    ...
    var questionEnglish =
        "Now I would like to ask you about liquids or foods children had yesterday during the day or at night. Did your child/children drink/eat: <u>Commercially produced infant formula</u>?";
    var questionTagalog =
        "Ano-anong uri ng pagkain at inumin ang binigay mo sa iyong mga anak kahapon mula umaga hanggang gabi? Kumain o uminom ang iyong mga anak ng: <u>Commercially produced infant formula</u>?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NU3commercialfood();
}
...
function NU3commercialfood() {
    ...
    var questionEnglish =
        "Now I would like to ask you about liquids or foods children had yesterday during the day or at night. Did your child/children drink/eat: <u>Any fortified, commercially available infant and young child food (e.g. Cerelac)</u>?";
    var questionTagalog =
        "Ano-anong uri ng pagkain at inumin ang binigay mo sa iyong mga anak kahapon mula umaga hanggang gabi? Kumain o uminom ang iyong mga anak ng: <u>Any fortified, commercially available infant and young child food (e.g. Cerelac)</u>?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NU3porridge();
}
...
function NU3porridge() {
    ...
    var questionEnglish =
        "Now I would like to ask you about liquids or foods children had yesterday during the day or at night. Did your child/children drink/eat: <u>Any (other) porridge or gruel</u>?";
    var questionTagalog =
        "Ano-anong uri ng pagkain at inumin ang binigay mo sa iyong mga anak kahapon mula umaga hanggang gabi? Kumain o uminom ang iyong mga anak ng: <u>Lugaw o am</u>?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    NU4();
}
...
function NU5() {
    var questionID = "NU5";
    var questionEnglish =
        "In the last 24 hours did you give <u>cereal</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>cereal</u>?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NU6();
}
...
function NU6() {
    var questionID = "NU6";
    var questionEnglish =
        "In the last 24 hours did you give <u>alm/rice</u> porridge to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>am o lugaw</u>?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NU7();
}
...
function NU7() {
    var questionID = "NU7";
    var questionEnglish =
        "In the last 24 hours did you give <u>vegetables</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>gulay</u>?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NU8();
}
...
function NU8() {
    var questionID = "NU8";
    var questionEnglish =
        "In the last 24 hours did you give <u>milk</u> to baby?";
    var questionTagalog =
        "Sa anakalipas na 24 oras pina-inom mo ba ang iyong sanggol ng <u>gatas</u>?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NU9();
}
...
function NU9() {
    var questionID = "NU9";
    var questionEnglish =
        "In the last 24 hours did you give <u>fruits</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>prutas</u>?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NU10();
}
...
function NU10() {
    var questionID = "NU10";
    var questionEnglish =
        "In the last 24 hours did you give <u>egg</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>itlog</u>?";
...
function NU11() {
    var questionID = "NU11";
    var questionEnglish =
        "In the last 24 hours did you give <u>fish</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>isda</u>?";
    analysisYesNoDontEat(questionID, questionEnglish, questionTagalog);
    NU12();
}
...
function NU12() {
    var questionID = "NU12";
    var questionEnglish =
        "In the last 24 hours did you give <u>meat</u> to baby?";
    var questionTagalog =
        "Sa nakalipas na 24 oras pinakain mo ba ang iyong sanggol ng <u>karne</u>?";
    analysisYesNoDontEat(questionID, questionEnglish, questionTagalog);
    NU13();
}
...
function NU13() {
    var questionID = "NU13";
    var questionEnglish =
        "What are the signs that a child that would suggest s/he was malnourished and should be referred to health facility?";
    var questionTagalog =
        "Ano ang mga sintomas na iyong makikita sa batang may malnutrisyon at kailangang madala at masuri agad sa isang pagamutan?";
    var answersEnglish = {
        "NU13-A": "underweight",
...
function IM1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_immunization'></span>Topic: Immunization and Vaccination Campaigns</h3><hr>"
    );
    var questionID = "IM1";
    var questionEnglish =
        "Do you have a card or child health booklet where vaccinations are written down?";
    var questionTagalog =
        "Mayroon ka bang card o child health booklet kung saan naka-rekord/nakatala ang nga bakuna na natanggap ng iyong mga anak?";
    var AyesCount = 0;
    var ByesCount = 0;
    var CnoCount = 0;
    var topicSkipped = 0;
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
            false) ? "<br><small>" + questionTagalog + "</small>" : "") +
        "</h4>" + "<p><strong>" + totalCount + " respondents</strong><br>" +
        "<span class='percText-1'>" + CnoPerc +
        "</span> no <span class='text-tagalog'>[wala]</span> (" + CnoCount.toString() +
        ")<br>" + "<span class='percText-2'>" + AyesPerc +
        "</span> yes, seen by interviewer <span class='text-tagalog'>[oo, nakita ng tagatanong]</span> (" +
        AyesCount.toString() + ")<br>" + "<span class='percText-3'>" +
        ByesPerc +
        "</span> yes, not seen <span class='text-tagalog'>[oo, ngunit hindi nakita]</span> (" +
        ByesCount.toString() + ")<br>";
    if (topicSkipped > 0) {
        thisInfoHtml += "(" + topicSkipped.toString() +
            " not asked this question)";
    }
    thisInfoHtml += "</p>";
    $(infoSelector).append(thisInfoHtml);
...
function IM2A() {
    ...
    var questionTagalog =
        "Nagkaroon ka ba ng vaccination card para sa iyong sanggol?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> Only asked if response to previous question was 'no card' <span class='text-tagalog'>[wala]</span><br>";
    IM2B();
}
...
function IM2B() {
    var questionID = "IM2B";
    var questionEnglish =
        "Where did you go to provide vaccines to your child/ children?";
    var questionTagalog =
        "Saan ka pumunta upang pabakunahan ang iyong anak/ mga anak?";
    var answersEnglish = {
        "IM2B-A": "your home",
        "IM2B-B": "midwife",
...
function IM2C() {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> Probe for the type(s) of person(s) and record all mentioned.<br>";
    $(infoSelector).append(thisNote);
    IM3();
}
...
function IM3() {
    ...
    var questionEnglish =
        "May I copy the information from the card? / May I ask you about vaccinations received by your child?";
    var questionTagalog = false;
...
function BCG() {
    ...
    var questionTagalog =
        "BCG (TB injection sa braso at kadalasan may peklat)";
...
function OPV0() {
    ...
    var questionEnglish =
        "POLIO 0 (drops given at birth or before 6 weeks)";
    var questionTagalog =
        "POLIO 0 (patak na ibinibigay pagkapanganak o bago mag 6 na linggo)";
    vaccineCard(questionID, questionEnglish, questionTagalog);
    OPV1();
}
...
function IM4() {
    var questionID = "IM4";
    var questionEnglish =
        "Can you tell me what diseases can be prevented using immunizations?";
    var questionTagalog =
        "Maaari mo bang sabihin kung ano ang mga sakit na maaaring maiwasan sa pamamagitan ng pagbabakuna?";
    var answersEnglish = {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> Do not read responses. Record all that are mentioned. " +
...
function WS1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_sanitation'></span>Topic: Safe Water, Hygiene, and Sanitation</h3><hr>"
    );
    var questionID = "cbhfa-WS_group-WS1";
    var questionEnglish =
        "What is the main source of drinking water for members of this household?";
    var questionTagalog =
        "Ano ang pangunahing pinagkukunan niyo ng inuming tubig sa inyong tahanan?";
    var answersEnglish = {
...
function WS2() {
    var questionID = "WS2";
    var questionEnglish =
        "Do you treat your water in any way to make it safer for drinking?";
    var questionTagalog =
        "May ginagawa ka bang paraan upang maging malinis at ligtas na inumin ang inyong tubig?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    WS3();
}
...
function WS3() {
    ...
    var questionTagalog =
        "Ano ang karaniwan mong ginagawa upang maging ligtas ang inyong iniinom na tubig?";
    var answersEnglish = {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> Only check more than one response, if several methods are usually used together, for example, cloth filtration and chlorine. " +
...
function WS4() {
    ...
    var questionTagalog =
        "Anong uri ng palikuran ang inyong ginagamit sa bahay?";
    var answersEnglish = {
...
function WS6number() {
    ...
    $("#infoWrapper").append('<div class="row"><div id="' + questionID +
        '_info" class="box-info"></div></div><hr>');
    var infoSelector = "#" + questionID + "_info";
    var thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<strong>" + numberResponses.length
        .toString() + " respondents providing # of people</strong><br>" +
        "Average number: " + avgNumber.toString() + "<br>" + "Min: " +
        minNumber.toString() + " / Max: " + maxNumber.toString() + "<br>" +
        "(" + notAskedCount.toString() + " not asked this question)";
    $(infoSelector).append(thisInfoHtml);
    WS7();
}
...
function WS7() {
    var questionID = "WS7";
    var questionEnglish =
        "May I see the toilet facility? Or can I use the toilet?";
...
function WS8A() {
    var questionID = "WS8A";
    var questionEnglish =
        "[elsewhere inside yard / outside yard] Toilet facility observation. Access to the facility… are there obstacles in the path, are there signs of regular use?";
    var questionTagalog =
        "[kahit saan sa loon ng bakuran / sa labas ng bakuran] Obserbahan ang daanan papunta sa pasilidad; mayroon bang mga harang sa daan, mayroon bang sinyales ng kadalasang pagkagamit?";
    var answersEnglish = {
    ...
    observeMultipleWhatAnswers(questionID, questionEnglish, questionTagalog,
        answersEnglish, answersTagalog);
    WS8B();
}
...
function WS8B() {
    var questionID = "WS8B";
    var questionEnglish =
        "[inside or attached to dwelling] Toilet facility observation. Access to the facility… are there obstacles in the path, are there signs of regular use?";
    var questionTagalog =
        "[sa loon ng bahay/tahanan] Obserbahan ang daanan papunta sa pasilidad; mayroon bang mga harang sa daan, mayroon bang sinyales ng kadalasang pagkagamit?";
    var answersEnglish = {
...
function WS9() {
    var questionID = "WS9";
    var questionEnglish =
        "Toilet facility observation: Is there faecal matter present inside the facility - on seat, floor, door, or walls (human or animal)?";
    var questionTagalog =
        "Pagmamasid sa palikuran/banyo: Mayroon bang dumi na nakakalat sa sahig, pinto, pader (tao o hayop)?";
...
function WS10() {
    var questionID = "WS10";
    var questionEnglish =
        "Toilet facility observation: Is there any overflow of the sewer or septic systems?";
    var questionTagalog =
        "Pagmamasid sa palikuran/banyo: Mayroon bang pag-apaw sa padalaluyan o septik?";
    analysisAssessYesNo(questionID, questionEnglish, questionTagalog);
    WS11();
}
...
function WS11() {
    var questionID = "WS11";
    var questionEnglish =
        "Toilet facility observation: Is there a cover on the hole?";
    var questionTagalog =
        "Pagmamasid sa palikuran/banyo: Mayroon bang takip ang butas?";
    analysisAssessYesNo(questionID, questionEnglish, questionTagalog);
    WS12();
}
...
function WS12() {
    var questionID = "WS12";
    var questionEnglish =
        "Can you show me where you usually wash your hands and what you use to wash hands? Or can I wash my hands?";
    var questionTagalog =
        "Maaari mo bang ipakita sakin kung saan kayo kadalasang naghuhugas ng kamay at kung ano ang inyong ginagamit sa paghuhugas? Or Maaari ba akong makihugas ng kamay?";
    var answersEnglish = {
...
function WS13() {
    ...
    var questionTagalog =
        "Pagmamasid lamang: Mayroon bang sabon o tradisyonal na ginagamit na panlinis?";
    var answersEnglish = {
        "A": "soap",
        "B": "detergent",
        "C": "mud or sand",
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> This item should be either in place or brought by the interviewee within one minute. If the item is not present within one minute check none, even if brought out later. " +
...
function WS14() {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> Turn on tap and/or a check container and note if water is present. " +
...
function WS15() {
    ...
    var questionTagalog =
        "Pagmamasid lamang: Mayroon bang kagamitan para sa paghuhugas ng kamay katulad ng gripo, palanggana, balde o lababo?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> This item should be either in place or brought by the interviewee within one minute. If the item is not present within one minute check none, even if brought out later. " +
...
function DI1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_diarrhoea'></span>Topic: Diarrhoea and Dehydation</h3><hr>"
    );
    var questionID = "DI1";
    var questionEnglish =
        "Has any family member had Diarrhea in the last 2 weeks? (define it if needed)";
    var questionTagalog =
        "Mayroon bang miyembro ng pamilya na nakaranas ng pagtatae sa nakaraang dalawang linggo? (ipalarawan kung kailangan)";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    DI2();
}
...
function DI2() {
    var questionID = "DI2";
    var questionEnglish =
        "If any family member was to suffer diarrhea (or when any family member did have diarrhea last) what did/ would you do? Anything else?";
    var questionTagalog =
        "Kung mayroong miyembro ng pamilya ang nakakaranas ng pagtatae, ano ang karaniwan mong ginagawa? May iba pa ba?";
    var answersEnglish = {
        "DI2-nothing": "nothing",
    ...
    var lessThreeDontKnowPerc = formatPerc((lessThanThree + nothingCount) /
        totalCount);
    var noResponsePerc = formatPerc(skipped / totalCount);
    var thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>" + totalCount +
        " respondents</strong><br>" + "<span class='percText-1'>" +
        atLeastThreePerc +
        "</span> could identify at least three key responses" + " (" +
        atLeastThree.toString() + ")<br>" + "<span class='percText-2'>" +
        lessThreeDontKnowPerc +
        "</span> could identify less than three key responses (" +
        lessThanThreePerc + ", " + lessThanThree.toString() +
        ") or nothing <span class='text-tagalog'>[wala]</span> " + " (" +
        nothingPerc + ", " + nothingCount.toString() + ")<br>" +
        "<span class='percText-3'>" + noResponsePerc +
        "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
        skipped.toString() + ")</p>";
    $(infoSelector).append(thisInfoHtml);
    $(infoSelector).append(
    ...
    for (response in allResponses) {
        ...
        var thisHtml = thisResponsePerc + " - " + thisResponseEng + ((
                thisResponseTag !== false) ?
            " <span class='text-tagalog'>[" + thisResponseTag +
            "]</span>" : "") + " (" + thisResponseCount + ")<br>";
        $(infoSelector).append(thisHtml);
    }
    $(infoSelector).append("(" + notAskedCount.toString() +
        " not asked this question)</p>");
    var thisNote =
        "<small><strong>Note:</strong> Do not read responses. Record all that are mentioned. " +
        "<span class='text-tagalog'>[Wag basahin ang mga pagpipilian. Itala lahat ng nabanggit.]</span><br>";
    $(infoSelector).append(thisNote);
...
function DI3() {
    var questionID = "DI3";
    var questionEnglish =
        "If your baby was to suffer, would/did you breastfeed him/her less than usual, about the same amount or more than usual?";
    var questionTagalog =
        "Kung nakakaranas ng pagtatae ang iyong sanggol, gaano mo siya kadalas pasusohin ng iyong gatas?";
    var answersEnglish = {
        "less": "less",
        "same": "same",
        "more": "more",
...
function DI4() {
    var questionID = "DI4";
    var questionEnglish =
        "If any family member was to suffer diarrhea (or when any family member did have diarrhea last), would/did you offer less than usual to <u>drink</u>, about the same amount, or more than usual to <u>drink</u>?";
    var questionTagalog =
        "Kung nakakaranas ng pagtataeang sinumang miyembro ng pamilya, gaano mo siya kadalas bibigyan ng <u>inumin</u>, gaya ng dati o mas marami sa <u>karaniwan</u>?";
    var answersEnglish = {
        "less": "less",
        "same": "same",
        "more": "more",
...
function DI5() {
    var questionID = "DI5";
    var questionEnglish =
        "If any family member was to suffer diarrhea (or when did any family member have diarrhea last), would/did you offer less than usual to <u>eat</u>, about the same amount, or more than usual to <u>eat</u>?";
    var questionTagalog =
        "Kung nakakaranas ng patatae ang isang miyembro ng pamilya, gaano mo siya kadalas <u>pakainin</u>, mas konti, kapareho o higit sa karaniwan niyang <u>kinakain</u>?";
    var answersEnglish = {
        "less": "less",
        "same": "same",
        "more": "more",
...
function DI6() {
    var questionID = "DI6"
    var questionEnglish =
        "Would/ did you seek advice or treatment from someone outside of the home for any family member who suffers from diarrhoea?";
    var questionTagalog =
        "Kung nakakaranas ng pagtatae ang kahit sinong miyembro ng pamilya, ikaw ba ay kukunsulta o kumukonsulta sa iba?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    DI7();
}
...
function DI7() {
    var questionID = "DI7";
    var questionEnglish =
        "Where would/ did you first go for advice or treatment?";
...
function DI9sachets() {
    ...
    var questionEnglish =
        "Ask mother to describe PREPARED SACHETS ORS preparation for you. Once mother has provided a description, record whether she described ors preparation correctly or incorrectly.";
    var questionTagalog =
        "Ipalarawan sa ina kung paano inihahanda ang NAKA-PAKETE ORS. Itala kung tama o hindi ang paglalarawan.";
    orsPrep(questionID, questionEnglish, questionTagalog);
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> described correctly if the mother mentioned the following... prepared sachets: use 1 liter of clean drinking water; use the entire packet; dissolve the powder fully" +
...
function DI9homemade() {
    ...
    var questionEnglish =
        "Ask mother to describe HOMEMADE ORS preparation for you. Once mother has provided a description, record whether she described ors preparation correctly or incorrectly.";
    var questionTagalog =
        "Ipalarawan sa ina kung paano inihahanda ang HOMEMADE ORS. Itala kung tama o hindi ang paglalarawan.";
    orsPrep(questionID, questionEnglish, questionTagalog);
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> described correctly if the mother mentioned the following... homemade: use 1 liter of clean drinking water; six level teaspoons of sugar; half level teaspoon of salt; stir until salt and sugar mixture is dissolved" +
...
function DI10() {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> multiple answers possible. ask: anything else? do not read responses. record all that are mentioned. " +
...
function DI11() {
    var questionID = "DI11";
    var questionEnglish =
        "Once the ORS is ready, for how long you can use that solution?";
    var questionTagalog =
        "Kung natimpla na ang ORS, gaano katagal ito pwedeng inumin?";
    var answersEnglish = {
        "A": "less than 8 hours ",
        "B": "8 -12 hours",
        "C": "12 – 24 hours",
...
function DI12() {
    var questionID = "DI12";
    var questionEnglish =
        "At what frequency ORS should be given to a child suffering from diarrhoea?";
    var questionTagalog =
        "Gaano kadalas ibinibigay ang ORS sa batang nakakaranas ng pagtatae?";
    var answersEnglish = {
        "A": "once a day",
        "B": "twice a day",
        "C": "thrice a day",
...
function DI13() {
    var questionID = "DI13";
    var questionEnglish =
        "How will you know that a child suffering from diarrhoea is dehydrated?";
    var questionTagalog =
        "Paano mo malalaman kung ang batang nakakaranas ng pagtatae ay dehydrated na?";
    var answersEnglish = {
...
function DI14() {
    ...
    var questionEnglish = "Do you know when to wash hands with soap?";
    var questionTagalog =
        "Tuwing kelan dapat maghugas ng kamay gamit ang sabon?";
    var answersEnglish = {
        "WS16-never": "never",
        "WS16-A": "after defecating",
        "WS16-B": "after urinating",
...
function AR1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_ari'></span>Topic: Acute Respiratory Infections</h3><hr>"
    );
    var questionID = "AR1";
    var questionEnglish =
        "What are the signs of pneumonia or ARI – acute respiratory infections – when a person should be taken immediately to a health facility?";
    var questionTagalog =
        "Ano ang mga senyales ng pneumonia o ARI – acute respiratory infections – at kalian dapat dalhin ang isang tao sa isang pagamutan upang masuri?";
    var answersEnglish = {
...
function AR2() {
    var questionID = "AR2";
    var questionEnglish =
        "Has any family member had an illness with a cough at any time in the last two weeks?";
    var questionTagalog =
        "May miymbro ban g pamilya na nagkasakit na may kasamang ubo sa nakalipas na 2 linggo?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    AR3();
}
...
function AR3() {
    var questionID = "AR3";
    var questionEnglish =
        "When any family member had an illness with a cough, did he/she have trouble breathing or breathe faster than usual with short, fast breaths?";
    var questionTagalog =
        "Kung nagkasakit at may kasamang ubo at kahit sino sa pamilya, nagkaroon ba sya ng problema sa paghinga?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    AR4();
}
...
function AR4() {
    var questionID = "AR4";
    var questionEnglish =
        "If any family member had a cough with fast breathing what would action would you take?";
    var questionTagalog =
        "Kung nakakaranas ng ubo si (pangalan) ano ang iyong ginagawa?";
    var answersEnglish = {
...
function AR5() {
    var questionID = "AR5";
    var questionEnglish =
        "How long after you noticed/ were noticing any family member cough and fast breathing did/ would you seek treatment?";
    var questionTagalog =
        "Gaano katagal bago mo napansin ang ubo at mabilis na pahinga ng bata at ipinagamot mo ba ito?";
    var answersEnglish = {
        "A": "same day",
        "B": "next day",
        "C": "two days",
...
function AR6() {
    var questionID = "AR6";
    var questionEnglish =
        "Where did (or if he/she has not been ill, would) you first go for advice or treatment?";
...
function AR7() {
    var questionID = "AR7";
    var questionEnglish =
        "How can you prevent childhood pneumonia (and ARIs – acute respiratory infections)?";
    var questionTagalog =
        "Paano mo maiiwasan ang pneumonia sa mga bata (at ARIs – acute respiratory infections)?";
    var answersEnglish = {
...
function malariaStart() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_malaria'></span>Topic: Malaria Prevention and Control</h3><hr>"
    );
...
function DN1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_dengue'></span>Topic: Dengue Prevention and Control</h3><hr>"
    );
    var questionID = "DN1";
...
function DN4() {
    var questionID = "DN4";
    var questionEnglish =
        "Do you know the time of the day which dengue mosquitoes bite?";
...
function DN6() {
    ...
    var questionTagalog =
        "Ano ang maaari mong gawin para makaiwas sa Dengue Fever?";
    var answersEnglish = {
...
function DN8() {
    var questionID = "DN8";
    var questionEnglish =
        "If you think that you have Dengue Fever, what do you do?";
    var questionTagalog =
        "Kung sa tingin mo na ikaw ay may Dengue Fever, ano ang maaari mong gawin?";
    var answersEnglish = {
...
function MN1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_mosquitonets'></span>Topic: Mosquito Net Use</h3><hr>"
    );
    var questionID = "MN1";
    var questionEnglish =
        "Does your household have any mosquito nets that can be used while sleeping?";
    var questionTagalog =
        "Gumagamit ba kayo ng kulambo sa inyong pagtulog?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    MN3();
}
...
function MN3() {
    ...
    var questionTagalog =
        "Nang makuha mo ang kasalukuyang kulambo, ito ba ay may pamatay peste?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    hiv();
}
...
function HA1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_hiv'></span>Topic: HIV and Sexually Transmitted Infections (STI)</h3><hr>"
    );
    var questionID = "HA1";
    var questionEnglish =
        "Have you ever heard of an illness called AIDS or an infection called HIV?";
    var questionTagalog =
        "Narinig mo na ba ang tungkol sa sakit na AIDS o ang impeksyon na HIV?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    tb();
}
...
function TB1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_tb'></span>Topic: Tuberculosis (TB)</h3><hr>"
    );
    var questionID = "TB1";
    var questionEnglish =
        "Have you heard about the disease called Tuberculosis or TB?";
    var questionTagalog =
        "Narinig mo na ba ang sakit na Tuberculosis o TB?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    TB3();
}
...
function TB3() {
    var questionID = "TB3";
    var questionEnglish =
        "Is TB contagious (can spread easily from one person to another)?";
...
function BD1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_blooddonation'></span>Topic: Blood Donation</h3><hr>"
    );
    var questionID = "BD1";
    var questionEnglish = "Have you donated blood in the last 12 months?";
    var questionTagalog =
        "Nag-donate ka ba ng dugo sa nakalipas na 12 buwan?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    BD2();
}
...
function BD2() {
    var questionID = "BD2";
    var questionEnglish =
        "Have any of your family member donated blood in the last 12 months?";
    var questionTagalog =
        "Mayroon bang miyembro ng pamilya na nag-donate ng dugo ng nakaraang 12 buwan?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    roadsafety();
}
...
function RS1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_roadsafety'></span>Topic: Road Safety</h3><hr>"
    );
    var questionID = "RS1";
    var questionEnglish =
        "Do you currently own a motorcycle or has one been provided for you to use?";
    var questionTagalog =
        "Sa kasalukuyan, ikaw ba ay nagmamay-ari ng isang motosiklo?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    RS2();
}
...
function RS2() {
    var questionID = "RS2";
    var questionEnglish =
        "How frequently do you wear a helmet when you are on a motorcycle?";
    var questionTagalog =
        "Gaano ka kadalas nagsusuot ng helmet  kapag ikaw ay nakasakay ng motorsiklo?";
    var answersEnglish = {
        "A": "always",
        "B": "usually",
        "C": "sometimes",
...
function RS3() {
    var questionID = "RS3";
    var questionEnglish =
        "The last time you rode a motorcycle did you wear a helmet?  ";
    var questionTagalog =
        "Noong huling sumakay ka ng motorsiklo, ikaw ba ay gumamit ng helmet?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog)
    RS4();
}
...
function RS4() {
    var questionID = "RS4";
    var questionEnglish =
        "The last time you rode a motorcycle did you fasten the chin strap on the helmet?";
    var questionTagalog =
        "Noong huli kang sumakay ng motorsiklo ikinabit mo ba ang chin strap sa helmet?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog)
    RS5();
}
...
function RS5() {
    var questionID = "RS5";
    var questionEnglish =
        "What actions make road users (walking, driving, riding) safer?";
    var questionTagalog =
        "Anaong aksyon ang maaring gawin ng mga gumgamit ng daan upang masigurado ang kanilang kaligtasan? (paglalakad, pagmamaneho, pagsakay sa sasakyan)?";
    var answersEnglish = {
...
function ES1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_substanceuse'></span>Topic: Excessive Substance Use</h3><hr>"
    );
    noncommunicablediseases();
}
...
function NC1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_noncommunicablediseases'></span>Topic: Noncommunicable Diseases</h3><hr>"
    );
    var questionID = "NC1";
    var questionEnglish =
        "Have you ever consumed an alcoholic drink such as wine, beer, spirit?";
    var questionTagalog =
        "Nakainom ka na ba ng mga inumin na may alcohol tulad ng wine, beer, spirit?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC2();
}
...
function NC2() {
    ...
    var questionTagalog =
        "Nakainom ka na ba ng inumin na may alcohol sa nakalipas na 12 buwan?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC3();
}
...
function NC3() {
    ...
    var questionTagalog =
        "Nakainom ka ba ng inuming may alcohol sa nakalipas na 30 araw?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC4();
}
...
function NC4() {
    var questionID = "NC4";
    var questionEnglish =
        "During each of the past 7 days, on how many occasions did you have at least one alcoholic drink?";
    var questionTagalog =
        "Sa bawat nakalipas na 7 araw, ilang beses ka nagkaroon ng pagkakataon na uminom ng alcohol?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    NC5();
}
...
function NC5() {
    var questionID = "NC5";
    var questionEnglish =
        "During the past 7 days, how many times did you have [for men: five or more / for women: four or more] standard alcoholic drinks in a single drinking occasion?";
    var questionTagalog =
        "Sa nakalipas na 7 araw, ilang beses ka uminom/ gaano [Sa lalaki: lima o higit pa / Sa babae: apat o higit pa] kadami ang iyong nainom kada araw?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    NC6();
}
...
function NC6() {
    var questionID = "NC6";
    var questionEnglish =
        "Does your work involve moderate-intensity activity that causes large increase in breathing or heart rate like carrying or lifting heavy loads, digging, harvesting for at least 10 minutes continuously?";
    var questionTagalog =
        "Ang iyong trabaho ba ay nangangailangan ng katamtamang lakas na mga gawain na nagdudulot ng pagbilis ng paghinga at pagbilis ng pintig ng puso kagaya ng pagdadala at pagbubuhat ng mabibigat na bagay, paghuhukay at pag-ani ng tuloy-tuloy sa loob ng kahit na 10 minuto?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC7();
}
...
function NC7() {
    ...
    var questionTagalog =
        "Sa tipikal na linggo, ilang araw ka gumagawa ng mga katamtamang lakas na mga gawain bilang parte ng iyong trabaho?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    NC9();
}
...
function NC9() {
    var questionID = "NC9";
    var questionEnglish =
        "Do you do any moderate-intensity sports, fitness or recreational activities (adult) that cause large increases in breathing or heart rate like running or football for at least 10 minutes continuously?";
    var questionTagalog =
        "May ginagawa ka bang katamtamang lakas na mga laro, gawaing pampalakas ng katawan, panlibangan (may sapat na gulang) na nagdudulot ng pagbilis ng paghinga at pintig ng puso kagaya ng pagtakbo o paglalaro ng football na tuloy-tuloy sa loob ng kahit na 10 minuto?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC10();
}
...
function NC10() {
    ...
    var questionTagalog =
        "Sa tipikal na linggo, ilang araw ka gumagawa ng mga katamtamang lakas na mga laro, gawaing pampalakas ng katawan o panlibangan?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    NC12();
}
...
function NC12() {
    ...
    var questionTagalog =
        "Sa kasalukuyan, ikaw ba ay naninigarilyo ng mga produkto ng tabako katulad ng cigarettes, cigars o pipes?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC13();
}
...
function NC13() {
    ...
    var questionTagalog =
        "Sa kasalukuyan, ikaw ba ay naninigarilyo ng tabako araw-araw?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC15();
}
...
function NC15() {
    ...
    var questionTagalog =
        "Nagkaroon ba ng pagkakataon na ipasuri mo sa duktor/nars ang iyong blood pressure?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC16();
}
...
function NC16() {
    ...
    var questionTagalog =
        "May pagkakataon bang na sinabihan ka ng duktor/nars na mataas ang iyong blood pressure?";
    analysisYesNo(questionID, questionEnglish, questionTagalog);
    NC17();
}
...
function NC18() {
    ...
    var questionTagalog =
        "Sa karaniwang linggo, ilang beses ka kumakain ng prutas?";
    analysisHowManyTimesPerWeek(questionID, questionEnglish,
        questionTagalog);
    NC19();
}
...
function NC19() {
    ...
    var questionTagalog =
        "Gaano kadami ang kinakain mong prutas sa mga araw na nabanggit?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    NC20();
}
...
function NC20() {
    ...
    var questionTagalog =
        "Sa karaniwang lingo, gaano ka kadalas kumain ng gulay?";
    analysisHowManyTimesPerWeek(questionID, questionEnglish,
        questionTagalog);
    NC21();
}
...
function NC21() {
    ...
    var questionTagalog =
        "Gaano kadami ang kinain mong gulay sa mga nabanggit na araw?";
    analysisHowManyTimes(questionID, questionEnglish, questionTagalog);
    violenceprevention();
}
...
function VP1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_violenceprevention'></span>Topic: Violence Prevention</h3><hr>"
    );
    var questionID = "VP1";
    var questionEnglish =
        'To what extent do you agree with the statement: "Violence against women, men, girls and boys is preventable."';
    var questionTagalog =
        'Sang-ayon ka ba o hindi sa panabing: "Ang karahasan sa mga kababaihan at kalalakihan ay maiiwasan."';
...
function VP2() {
    var questionID = "VP2";
    var questionEnglish =
        'To what extent do you agree with the statement: "There are certain situations in a family when it is okay to hit someone else."';
    var questionTagalog =
        'Sang-ayon ka ba o hindi sa panabing: "May mga pagkakataon na maaaring paluin o saktan ng pisikal ang isang tao sa pamilya."';
    analysisAgreeDisagree(questionID, questionEnglish, questionTagalog);
    VP3();
}
...
function VP3() {
    var questionID = "VP3";
    var questionEnglish =
        'To what extent do you agree with the statement: "A woman always has the right to refuse sexual contact."';
    var questionTagalog =
        'Sang-ayon ka ba o hindi sa panabing: "Ang isang babae ay may karapatang tumanggi sa pakikipagtalik."';
    analysisAgreeDisagree(questionID, questionEnglish, questionTagalog);
    VP4();
}
...
function VP4() {
    var questionID = "VP4";
    var questionEnglish =
        'To what extent do you agree with the statement: "Constantly insulting another person is a form of violence."';
    var questionTagalog =
        'Sang-ayon ka ba o hindi sa panabing: "Ang madalas na pag-iinsulto sa isang tao ay uri ng karahasan."';
    analysisAgreeDisagree(questionID, questionEnglish, questionTagalog);
    VP5();
}
...
function VP5() {
    var questionID = "VP5";
    var questionEnglish =
        'To what extent do you agree with the statement: "People who see or hear violence occurring have an important role to stop the violence when it is safe to do so."';
    var questionTagalog =
        'Sang-ayon ka ba o hindi sa panabing: "Sinuman ang nakakakita o nakakarinig ng kahit anong uri ng karahasan ay may importanteng ganap upang maitigil o maihinto sa ligtas na sitwasyon."';
    analysisAgreeDisagree(questionID, questionEnglish, questionTagalog);
    VP6();
}
...
function VP6() {
    var questionID = "VP6";
    var questionEnglish =
        "In your opinion, what are the safest ways to discipline children?";
    var questionTagalog =
        "Sa iyong opinion, ano ang mga ligtas na paraan upang disiplinahin ang mga bata?";
    var answersEnglish = {
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisNote =
        "<small><strong>Note:</strong> do not prompt respondents. let them know they can provide more than one answer. record all that are mentioned." +
...
function VP7() {
    ...
    var questionTagalog =
        "Ano ang epekto o mga epekto ng karahasan sa mga tao?";
    var answersEnglish = {
...
function VP8() {
    var questionID = "VP8";
    var questionEnglish =
        "If you saw or heard someone being sexually violent against another person, what immediate action could you take?";
    var questionTagalog =
        "Kung ikaw ay nakakita o nakarinig ng isang taong sekswal na nangangahas ng ibang tao, ano ang panandaliang aksyon na dapat mong gawin?";
    var answersEnglish = {
...
function VP9() {
    var questionID = "VP9";
    var questionEnglish =
        "What practical action can you take to prevent violence in disaster?";
    var questionTagalog =
        "Ano ang maaari mong gawin upang maiwasan ang karahasan sa panahon ng kalamidad?";
    var answersEnglish = {
...
function VP10() {
    var questionID = "VP10";
    var questionEnglish =
        "If a person tells you they are being hurt by violence what can you do to help the person?";
    var questionTagalog =
        "Kung ang isang taong biktima ng karahasan ay lumapit sayo upang humingi ng tulong, ano ang maaari mong gawin upang siya ay matulungan?";
    var answersEnglish = {
...
function RC1() {
    $(infoWrapper).append(
        "<h3><span class='jumpto' id='section_redcross'></span>Topic: Red Cross Exposure</h3><hr>"
    );
    var questionID = "RC1";
    var questionEnglish = "Are you aware about Philippine Red Cross?";
    var questionTagalog =
        "May kaalaman ka ba tungkol sa Philippine Red Cross?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    RC2();
}
...
function RC2() {
    var questionID = "RC2";
    var questionEnglish =
        "In the last 1 year, has your household received a visit from a PRC volunteer?";
    var questionTagalog =
        "Sa nakaraang taon, ang iyong sambahayan ay binisita ng isang PRC volunteer?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    RC4();
}
...
function RC4() {
    var questionID = "RC4";
    var questionEnglish =
        "Did the PRC volunteer discuss with you or someone in your household any of the following subjects:";
    var questionTagalog =
        "Tinalakay ba ng Red Cross volunteer ang mga sumusunod na paksa sa iyo at ng iyong pamilya:";
    var answersEnglish = {
...
function RC5() {
    var questionID = "RC5";
    var questionEnglish =
        "Did you talk about what was discussed by the volunteer with any other family members or friends?";
    var questionTagalog =
        "Ibinahagai mo ba ang mga tinalakay ng volunteer sa mga miyembro ng iyong pamilya o mga kaibigan?";
    analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    RC6();
};
...
function RC6() {
        var questionID = "RC6";
        var questionEnglish =
            "Did you participate in any activity conducted by PRC? ";
        var questionTagalog =
            "Ikaw ba ay nakilahok sa talakayang ginawa ng Red Cross/Red Crescent?";
        analysisYesNoDk(questionID, questionEnglish, questionTagalog);
    }
    // "IM2B-A":"",
    // "IM2B-B":"",
...
function analysisYesNo(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
            false) ? "<br><small>" + questionTagalog + "</small>" : "") +
        "</h4>" + "<p><strong>" + totalCount + " respondents</strong><br>" +
        "<span class='percText-1'>" + yesPerc +
        "</span> yes <span class='text-tagalog'>[oo]</span> (" + yesCount.toString() +
        ")<br>" + "<span class='percText-2'>" + noPerc +
        "</span> no <span class='text-tagalog'>[hindi]</span> (" + noCount.toString() +
        ")<br>" + "<span class='percText-3'>" + noResponsePerc +
        "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
        skipped.toString() + ")<br>";
    if (topicSkipped > 0) {
        thisInfoHtml += "(" + topicSkipped.toString() +
...
function analysisYesNoDk(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
        ...
        "<span class='percText-1'>" + yesPerc +
        "</span> yes <span class='text-tagalog'>[oo]</span> (" + yesCount.toString() +
        ")<br>" + "<span class='percText-2'>" + nodkPerc +
        "</span> no <span class='text-tagalog'>[hindi]</span> (" + noPerc +
        ", " + noCount +
...
function analysisYesNoDontEat(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
        ...
        "<span class='percText-1'>" + yesPerc +
        "</span> yes <span class='text-tagalog'>[oo]</span> (" + yesCount.toString() +
        ")<br>" + "<span class='percText-2'>" + nodonteatPerc +
        "</span> no <span class='text-tagalog'>[hindi]</span> (" + noPerc +
        ", " + noCount +
        ") or don't eat <span class='text-tagalog'>[hindi kumakain]</span> (" +
        donteatPerc + ", " + dontEatCount.toString() + ")<br>" +
        "<span class='percText-3'>" + skipPerc +
        "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
        skipped.toString() + ")<br>";
    if (topicSkipped > 0) {
...
function analysisSelectOneWhatAnswer(questionID, questionEnglish,
    questionTagalog, answersEnglish, answersTagalog) {
    var topicSkipped = 0;
    var totalCount = 0;
    var allResponses = [];
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
        ...
        "</h4>";
    $(infoSelector).append(thisInfoHtml);
    $(infoSelector).append("<p><strong>" + totalCount.toString() +
        " respondents (single response)</strong><br>");
    for (response in allResponses) {
        var thisResponseCount = allResponses[response];
        var thisResponsePerc = formatPerc(allResponses[response] /
            totalCount);
        ...
        var thisHtml = thisResponsePerc + " - " + thisEnglish + ((
                thisTagalog !== false) ?
            " <span class='text-tagalog'>[" + thisTagalog + "]</span>" :
            "") + " (" + thisResponseCount + ")<br>";
        $(infoSelector).append(thisHtml);
    }
    $(infoSelector).append("(" + topicSkipped +
        " not asked this question)</p>");
}

function analysisSelectMultipleWhatAnswers(questionID, questionEnglish,
    questionTagalog, answersEnglish, answersTagalog) {
    var skip = questionID + "-skip";
    var totalCount = 0;
    var topicSkipped = 0;
    ...
    $(infoSelector).append("(" + topicSkipped.toString() +
        " respondents not asked this question)</p>");
}

function observeMultipleWhatAnswers(questionID, questionEnglish,
    questionTagalog, answersEnglish, answersTagalog) {
...
function analysisHowManyTimes(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "";
    thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>" + numberResponses
        .length.toString() + " respondents providing #</strong><br>" +
        "Average Times: " + avgTimes.toString() + "<br>" + "Min: " +
        minTimes.toString() + " / Max: " + maxTimes.toString() + "<br>" +
        "(" + dkCount.toString() +
        " don't know <span class='text-tagalog'>[hindi alam]</span>, " +
...
function analysisHowManyTimesPerWeek(questionID, questionEnglish,
    questionTagalog) {
    var numberResponses = [];
    var dkCount = 0;
    var noResponseCount = 0;
...
function analysisMoreThreeLessThree(questionID, questionEnglish,
    questionTagalog, answersEnglish, answersTagalog, optionCount) {
    var atLeastThree = 0;
    var lessThanThree = 0;
    var dontKnow = 0;
    var skipped = 0;
    ...
    var lessThreeDontKnowPerc = formatPerc((lessThanThree + dontKnow) /
        totalCount);
    var noResponsePerc = formatPerc(skipped / totalCount);
    var thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>" + totalCount +
        ...
        "</span> could identify less than three key responses (" +
        lessThanThreePerc + ", " + lessThanThree.toString() +
        ") or don't know <span class='text-tagalog'>[walang sagot]</span> " +
...
function analysisAssessYesNo(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4>" + "<p><strong>" + totalCount +
        " observations</strong><br>" + "<span class='percText-1'>" + noPerc +
        "</span> no <span class='text-tagalog'>[hindi]</span> (" + noCount.toString() +
        ") <br>" + "<span class='percText-2'>" + yesPerc +
        "</span> yes <span class='text-tagalog'>[oo]</span> (" + yesCount.toString() +
        ")<br>" + "<span class='percText-3'>" + cannotPerc +
        "</span> cannot assess <span class='text-tagalog'>[hindi masuri]</span> (" +
        cannotObserveCount.toString() + ")<br>" + "(" + notAskedCount.toString() +
...
function analysisAgreeDisagree(questionID, questionEnglish, questionTagalog) {
    ...
    $.each(filteredData, function(surveyIndex, survey) {
        if (survey[questionID] === "n/a") {
            ...
            totalCount++;
            if (survey[questionID] === "agree") {
                agreeCount++;
            }
            if (survey[questionID] === "neither") {
                neitherCount++;
            }
            if (survey[questionID] === "disagree") {
                disagreeCount++;
            }
            if (survey[questionID] === "dk") {
                dkCount++;
    ...
    var infoSelector = "#" + questionID + "_info";
    var thisInfoHtml = "<h4>" + questionEnglish + "<br><small>" +
        questionTagalog + "</small></h4><br>" + '<div id="' + questionID +
        '_bar" class="question-block">' + '<div class="responsesBar">' +
        '<svg width="100%" height="30">' +
        '<rect class="response-bar noResponse" y="0" height="100%" width="' +
        noResponse + '" ></rect>' +
        '<rect class="response-bar dk" y="0" height="100%" width="' + dk +
        '" ></rect>' +
        '<rect class="response-bar disagree" y="0" height="100%" width="' +
        disagree + '" ></rect>' +
        '<rect class="response-bar neither" y="0" height="100%" width="' +
        neither + '" ></rect>' +
        '<rect class="response-bar agree" y="0" height="100%" width="' +
        agree + '" ></rect>' + '</svg></div></div>' + "<p><strong>" +
        totalCount + " respondents</strong><br>" +
        "<span class='text-agree'>" + formatPerc(agreeCount / totalCount) +
        "</span> agree <span class='text-tagalog'>[sang-ayon]</span> (" +
        agreeCount.toString() + ")<br>" + "<span class='text-neither'>" +
        formatPerc(neitherCount / totalCount) +
        "</span> neither <span class='text-tagalog'>[wala sa anuman]</span> (" +
        neitherCount.toString() + ")<br>" + "<span class='text-disagree'>" +
        formatPerc(disagreeCount / totalCount) +
        "</span> disagree <span class='text-tagalog'>[hindi sang-ayon]</span> (" +
        disagreeCount.toString() + ")<br>" + "<span class='text-dk'>" +
        formatPerc(dkCount / totalCount) +
        "</span> don't know <span class='text-tagalog'>[hindi alam]</span> (" +
        dkCount.toString() + ")<br>" + "<span class='text-noResponse'>" +
        formatPerc(noResponseCount / totalCount) +
        "</span> no response <span class='text-tagalog'>[walang sagot]</span> (" +
        noResponseCount.toString() + ")<br>" + "(" + notAskedCount.toString() +
        " respondents not asked this question)</p>";
    $(infoSelector).append(thisInfoHtml);
}
...
function vaccineCard(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
            false) ? "<br><small>" + questionTagalog + "</small>" : "") +
        "</h4>" + "<p><strong>" + totalCount +
        " observed/asked</strong><br>" + "<span class='percText-1'>" +
        nodatePerc +
        "</span> card has no date OR no card and not recalls child receiving (" +
        nodateCount.toString() + ") <br>" + "<span class='percText-2'>" +
        datePerc + "</span> card has date (" + dateCount.toString() +
        ")<br>" + "<span class='percText-3'>" + recallsPerc +
        "</span> no card or no date on card but recalls child receiving (" +
        recallsCount.toString() + ")<br>" + "(" + notAskedCount.toString() +
        " not observed/asked)";
    $(infoSelector).append(thisInfoHtml);
}
...
function observationStartOptions(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
        ...
        "</h4>" + "<p><strong>" + totalCount + " respondents</strong><br>" +
        "<span class='percText-1'>" + yesPerc +
        "</span> yes, permission is given <span class='text-tagalog'>[oo, may pahintulot]</span> (" +
        yesCount.toString() + ")<br>" + "<span class='percText-2'>" +
        noPerc +
        "</span> no, permission is not given <span class='text-tagalog'>[hindi, walang pahintulot]</span> (" +
        noCount.toString() + ")<br>";
    if (topicSkipped > 0) {
        thisInfoHtml += "(" + topicSkipped.toString() +
            " not asked this question)";
...
function orsPrep(questionID, questionEnglish, questionTagalog) {
    ...
    var thisInfoHtml = "<h4>" + questionEnglish + ((questionTagalog !==
        ...
        "</h4>" + "<p><strong>" + totalCount + " respondents</strong><br>" +
        "<span class='percText-1'>" + yesPerc +
        "</span> described correctly <span class='text-tagalog'>[tamang paglalarawan]</span> (" +
        yesCount.toString() + ")<br>" + "<span class='percText-2'>" +
        nodkPerc +
        "</span> described incorrectly <span class='text-tagalog'>[maling paglalarawan]</span> (" +
        noPerc + ", " + noCount +
        ") or don't know <span class='text-tagalog'>[hindi alam]</span> (" +
        dkPerc + ", " + dontknowCount.toString() + ")<br>" +
        "<span class='percText-3'>" + skipPerc +
